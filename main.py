...
def main():
    ...
    for f in step1_files:
        ...
        if "(Instrumental)" in f:
            rename_file(path, "1_纯伴奏_Instrumental.flac")
        elif "(Vocals)" in f:
            # 暂时命名为临时文件
            full_vocals_path = rename_file(path, "temp_full_vocals.flac")

    # =======================================================
    # [Stage 2] 主唱/和声分离
    # 模型：UVR_MDXNET_KARA_2
    # =======================================================
    lead_vocals_path = None
    if full_vocals_path:
        print("\n[2/3] 正在分离 主唱 与 和声...")
        # 换成新一代 Mel-Band Roformer karaoke 模型，对紧贴主旋律的和声分离效果远好于旧的 MDX-Net
        ...
        for f in step2_files:
            ...
            if "(Instrumental)" in f:
                ...
                rename_file(path, "3_纯和声_Backing_Vocals.flac")
            elif "(Vocals)" in f:
                # 这是一个中间产物：带混响的主唱
                lead_vocals_path = rename_file(path, "temp_lead_vocals_reverb.flac")
    ...
    if lead_vocals_path:
        print("\n[3/3] 正在对主唱进行【深度去混响】...")
        print("💡 正在加载 VR DeEcho 模型 (首次下载约 40MB)...")
        ...
        for f in step3_files:
            ...
            if "(Reverb)" in f and "(No Reverb)" not in f:
                # 这是被切下来的混响声，你可以留着做特效，或者删掉
                rename_file(path, "5_被切除的混响_Reverb_Only.flac")
            elif "(No Reverb)" in f:
                # 这就是你要的终极干声！
                rename_file(path, "2_纯主唱_干声_Dry_Vocals.flac")
